...
class TP04q01 {
     ...
     * e insere/remove jogos da Lista de acordo com parametros da entrada,
     * mostrando os removidos e a Lista ao final
     */
    public static void main(String[] args) throws Exception {
        String file = "/tmp/games.csv";
        BufferedReader input = new BufferedReader(new InputStreamReader(System.in));
        
        Lista listaGames = new Lista();
        ...
        for(int i = 0; i < qtd; i++){
            ...
            if(comando.contains("II")){ // inserir no inicio
                id = Integer.parseInt( comando.substring(3, comando.length()) );
                
                game.readFromFileID(file, id);
                listaGames.inserirInicio(game);
            } else if(comando.contains("IF")){ // inserir no fim
                id = Integer.parseInt( comando.substring(3, comando.length()) );
                
                game.readFromFileID(file, id);
                listaGames.inserirFim(game);
            } else if(comando.contains("I*")){ // inserir em posicao
                pos = Integer.parseInt( comando.substring(3,5) );
                id = Integer.parseInt( comando.substring(6, comando.length()) );

                game.readFromFileID(file, id);
                listaGames.inserir(game, pos);
            } else if(comando.contains("RI")){ // remover no inicio
                System.out.println("(R) "+listaGames.removerInicio().getName());
            } else if(comando.contains("RF")){ // remover no fim
                System.out.println("(R) "+listaGames.removerFim().getName());
            } else if(comando.contains("R*")){ // remover em posicao
                pos = Integer.parseInt( comando.substring(3,5) );
                
                System.out.println("(R) "+listaGames.remover(pos).getName());
            }
        }
