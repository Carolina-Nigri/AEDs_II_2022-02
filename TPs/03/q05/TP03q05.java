...
class Lista {
    ...
    /**
     * Ordena o array pelo release_date do Game (Heapsort)
     */    
    public void ordena() {
        comp = mov = 0;

        // cria um array temporario a partir da posicao 1 com o array original 
        ...
        // salva array temporario no original
        array = tmp;

        // constroi o heap, adicionando um elemento a cada iteracao
        ...
        while(tamHeap > 1){
            // troca raiz com a ultima folha do ultimo nivel  
            swap(1, tamHeap);
            mov+=3;
            tamHeap--;
        ...
        // colocar o array comecando da posicao 0 novamente
        tmp = array;
        array = new Game[n];
    ...
    // constroi o heap invertido, verificando se filhos sao menores que pais
    public void construir(int tamHeap){
        int i = tamHeap;

        comp++;
        // verifica se filho vem depois do pai e, se verdadeiro, troca os dois
        while( i > 1 && ( array[i].getRelease_date().after(array[i/2].getRelease_date()) ||
                          array[i].getRelease_date().equals(array[i/2].getRelease_date()) 
                                && vemAntes(array[i/2].getName(), array[i].getName()) ) ){
    ...
    // reconstroi o heap invertido, verificando se pais sao maiores que os filhos
    public void reconstruir(int tamHeap){
        ...
        while( i <= (tamHeap/2) ){
            ...
            comp++;
            // verifica se pai vem antes do filho e troca os dois se verdadeiro
            if( array[i].getRelease_date().before(array[filho].getRelease_date()) ||
                array[i].getRelease_date().equals(array[filho].getRelease_date())
                    && vemAntes(array[i].getName(), array[filho].getName()) ){
                ...
                i = tamHeap; // finaliza reconstrucao
            }
        }
    }

    // retorna posicao do maior filho do heap
    public int getMaiorFilho(int i, int tamHeap){
        int filho;
        
        comp++;
        // (2*i) eh o unico filho (ultima folha) OU eh maior que o filho (2*i+1)
        if( (2*i) == tamHeap || 
            array[2*i].getRelease_date().after(array[2*i+1].getRelease_date()) ||
            array[2*i].getRelease_date().equals(array[2*i+1].getRelease_date()) 
                && vemAntes(array[2*i+1].getName(), array[2*i].getName()) ){
           
            filho = 2*i;
        } else{ // (2*i+1) eh o maior filho
           filho = 2*i + 1;
        }
