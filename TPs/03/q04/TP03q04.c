...
// Prototipos
void ordena();
void insercaoPorGrupo(int grupo, int h);
void mostrar();
...
void ordena(){ 
    ...
    int h = 1; // qtd de grupos

    // acha primeiro valor da sequencia de Knuth maior que n 
    do{ 
        h = (h * 3) + 1; 
    } while(h < n);

    do{
        h /= 3; // qtd de grupos (atual / 3 => termo anterior da sequencia de Knuth)
        
        for(int grupo = 0; grupo < h; grupo++){
            insercaoPorGrupo(grupo, h);
        }
    } while(h != 1); // faz ordenacoes por insercao por grupos ate ter somente 1 grupo
}
void insercaoPorGrupo(int grupo, int h){
    // faz a ordenacao pelo algoritmo de insercao no grupo x dos h grupos
    for(int i = (h + grupo); i < n; i += h) {
        Game tmp = array[i];
        mov++;

        int j = i - h; // posicao do elemento anterior do grupo x 
        
        comp++;
        while( (j >= 0) && (array[j].app_id > tmp.app_id) ){
            array[j + h] = array[j];
            mov++;
