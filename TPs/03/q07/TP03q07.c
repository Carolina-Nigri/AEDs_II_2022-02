...
int comp, mov; // número de comparações e movimentações da ordenação
// -------------------------------------------------------------------------------- //
// Prototipos
void ordena();
int getMaior();
...
int main(){
    ...
    FILE* logFile = fopen("matricula_countingsort.txt","w");
    ...
    // le ids de games ate achar FIM e salva na Lista
    while(!isFim(gameID)){        
        id = atoi(gameID);
...
void ordena(){ 
    ...
    int count[tamCount]; // array de contagem
    Game ordenado[n]; // array ordenado

    // inicializar cada posicao do array de contagem como 0
    for(int i = 0; i < tamCount; i++) 
        count[i] = 0;

    // salva no array de contagem, na posicao i, o numero de elementos iguais a i
    for(int i = 0; i < n; i++) 
        count[(int)(array[i].upvotes * 1000)]++;

    // salva no array de contagem, na posicao i, o numero de elementos menores que i
    for(int i = 1; i < tamCount; i++)
        count[i] += count[i-1];

    // ordenando
    for(int i = n-1; i >= 0; i--){
        int pos = count[(int)(array[i].upvotes * 1000)] - 1;
...
int getMaior(){
    int maior = (int)(array[0].upvotes * 1000);
    char *nome = array[0].name;

    for(int i = 0; i < n; i++) {
        comp++;

        if( (int)(array[i].upvotes * 1000) == maior && vemAntes(nome, array[i].name) ){
            maior = (int)(array[i].upvotes * 1000);
            nome = array[i].name;
        } else if( (int)(array[i].upvotes * 1000) > maior ){
            maior = (int)(array[i].upvotes * 1000);
            nome = array[i].name;
        }
    }
