...
int main(){
    ...
    for(int i = 0; i < qtd; i++){
        ...
        if(comando[0] ==  'I'){ // inserir
            if(comando[1] == 'I'){ // inicio
                substring(sub, &comando[3], (strlen(comando) - 3));
                id = atoi(sub);
                
                readFromFileID(csvFile, &game, id);
                inserirInicio(game);
            } else if(comando[1] == 'F'){ // fim
                substring(sub, &comando[3], (strlen(comando) - 3));
                id = atoi(sub);
                
                readFromFileID(csvFile, &game, id);
                inserirFim(game);
            } else{ // posicao especifica
                substring(sub, &comando[3], 2);
                pos = atoi(sub);
                
                substring(sub, &comando[6], (strlen(comando) - 6));
                id = atoi(sub);
