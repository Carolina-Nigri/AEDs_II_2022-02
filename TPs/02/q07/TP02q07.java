...
class FilaCircular {
    // atributos
    private Game[] array;
    private int primeiro; // aponta para primeira posicao (logica)
    private int n; // qtd de elementos na Fila
    ...
    public int mediaArredAvgPt() {
        int soma = 0, media = 0, j = 0, i = primeiro;
        
        while( j < n ){
            soma += array[i].getAvg_pt();

            i = (i + 1) % array.length;
            j++;
        }

        media = Math.round( (float) soma / n );
    ...
    public void inserir(Game game) throws Exception {
        ...
        array[(n + primeiro) % array.length] = game;
        n++;
    }
